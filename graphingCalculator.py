...
# Main procedure to draw f(x), first derivative, second derivative, and mark min/max/inflection/removable discontinuity points
# Return: Indicator whether f(x) has any removable discontinuity points
def graph(x, plotGraph = True):
    # rounding precision
    rounding_number = 2
    ...
    for i in range(0, n):
        if (np.isnan(y[i]) or np.isinf(y[i])):
            if (i >= 2 and i <= n - 2):
                ...
                multi_value = (y[i-1] - y[i-2]) * (y[i+1] - y[i-1])
                if (multi_value <= 0.0):
                ...
                multi_value = (y[i+2] - y[i+1]) * (y[i+1] - y[i-1])
                if (multi_value <= 0.0):
                    #print "Nonremovable discontinuity point:", x[i]
                    s = "Nonremovable discontinuity point:" + str(x[i])
                    has_non_removable_discontinuity_point = True
...
# Use the integral range to find out whether there is any non removable discontinuity point
#xpoints = np.linspace(a, b, n + 1)
#reset xpoints according to specified starting and ending value for the integral
#this is to check for nonremdis in the interval
xpoints = np.linspace(a, b, (b-a)*10000)
#only detects nonremdis in graph but doesnt actually graph it a second time
has_non_removable_discontinuity_point = graph(xpoints, False)
print ""

# If the graph has non removable discontinuity point, no FTC
#hasnonremdis has to check throughout the user input right now
